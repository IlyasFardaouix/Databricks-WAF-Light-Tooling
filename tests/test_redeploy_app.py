...
def test_get_workspace_path_from_args():
    ...
    with patch.object(
        sys, "argv", ["test_redeploy_app.py", "app_name", "/path/to/workspace"]
    ):
    ...
    with patch.object(sys, "argv", ["test_redeploy_app.py", "app_name"]):
        assert (
            get_workspace_path_from_args()
            == f"/Users/<your_user>@databricks.com/app_name"
        )
...
def test_get_databricks_credentials():
    ...
    with patch.dict(
        "os.environ",
        {"DATABRICKS_HOST": "https://example.com", "DATABRICKS_TOKEN": "token"},
    ):
        assert get_databricks_credentials() == {
...
def test_redeploy_app():
    ...
    with patch.object(requests, "post") as mock_post:
        ...
        with pytest.raises(SystemExit):
            redeploy_app(
                "app_name", "/path/to/workspace", "https://example.com", "token"
            )
